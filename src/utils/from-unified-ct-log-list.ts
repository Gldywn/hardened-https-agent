...
export function fromUnifiedCtLogList(logList: UnifiedCTLogList, logType: 'prod' | 'test' = 'prod'): Log[] {
  ...
  for (const operator of logList.operators) {
    ...
    for (const ctLog of operator.logs as CTLog[]) {
      if (ctLog.log_type && ctLog.log_type !== logType) continue; // If the log is not of the specified type, skip it
